...
זה חל על הפוסט כולו: גוף הפוסט וההנעה לפעולה, כולל הטיות הפעלים (${addressGender === "feminine" ? '"הגיבי", "אשלח לך", "תקבלי"' : addressGender === "masculine" ? '"הגב", "אשלח לך", "תקבל"' : '"הגיבו", "אשלח לכם", "תקבלו"'}).`
...
הגוף שקבעת חל על הפוסט כולו: גוף הפוסט וההנעה לפעולה, כולל הטיות הפעלים ("הגיבי / הגב / הגיבו", "אשלח לך / לכם").`}
...
### 2. שורת אוטוריטה: רק אם המשתמש כתב אותה בעצמו
**אל תמציא שורת אוטוריטה.** משפטים כמו "אחרי 15 שנה בתחום למדתי ש...", "אני עושה את זה כל יום", "ראיתי את זה אצל מאות לקוחות" אסורים, אלא אם הם מופיעים בטקסט של המשתמש (סעיף 3 למטה).
- אם המשתמש כתב בעצמו משפט שמציג את הניסיון או הסמכות שלו, השאר אותו כלשונו, במקום שבו הוא נמצא בטקסט שלו (או כשורה שנייה אם זה מתאים לזרימה).
- אם לא כתב, אין שורת אוטוריטה בכלל. עוברים ישירות מההוק לגוף הפוסט.
...
7. **אפס מקפים ארוכים.** אסור להשתמש ב-em dash או ב-en dash בשום מקום בפוסט, ואסור לשים שני מקפים רצופים. כשצריך לחבר שני חלקי משפט, השתמש בפסיק, בנקודה, בנקודתיים או בשורה חדשה. מקף קצר רגיל מותר רק בתוך מילה או צירוף (למשל "ה-AI", "בין-לאומי"), אף פעם לא כמפריד בין חלקי משפט. שים לב: ההנחיות האלה עצמן כתובות בלי מקפים ארוכים, וכך גם הפוסט.
8. **אסור לכתוב בתבנית "זה לא X, זה Y" / "זה לא X אלא Y".** התבנית הזו שחוקה ונשמעת מלאכותית. במקום זה, אמור ישירות מה זה כן ("בעצם זה Y", "מה שקורה כאן זה Y", "האמת היא Y"), או הצג רק את ה-Y בלי לפסול במפורש את ה-X.
